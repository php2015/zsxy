...
class Notify extends \WxPayNotify
{
    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        // 订单状态未修改情况下, 进行处理业务
        $result = $this->processOrders($order, $data);
        if(!$result) {
            $msg = '订单处理失败';
            return false;
        }
			...
			//查询版本人信息
             ...
             $chaxun_data=[
              ...
              if($chaxun_id)
              {
					...
					$c = db('chaxun')->where('uid','=',$sales['uid'])->where('dates','between',[$kaishitime,time()])->order('dates asc')->find();
                                
					
					if(isset($c) && !empty($c)){
						db('chaxun')->where('id','=',$chaxun_id)->update(['dates'=>$c['dates']]);
					}
     ...
     * @return Bollean
     */
    public function processOrders($order, $data)
    {
        // 进行核心业务处理, 如更新状态, 发送通知等等
        // 处理成功, 返回true, 处理失败, 返回false
        // 例如:
  ...
  function fenyong($dingdanid){
        ...
        $shangji=db('user')->where('id','=',$tichenguser['pid'])->find();
     	...
     	file_put_contents("7777.txt",$tichenguser['isStaff']);
