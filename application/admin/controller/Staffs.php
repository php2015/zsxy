...
class Staffs extends AdminBase{
    ...
    public function index($keyword = '', $page = 1,$date1='',$date2='',$tag=0){
        ...
        $agentUser = Db::name('User')->field('id')->where('isStaff','=',1)->select();
        ...
        if (!empty($keyword)) {
            ...
            //总成交数量
            $zfsum = Db::name('sales')->field('id')->where('sessionfpid','in',$apid)->where('status','=',1)->where($ma)->count('id');
            $wzfsum = Db::name('sales')->field('id')->where('sessionfpid','in',$apid)->where('status','=',0)->where($ma)->count('id');
            //当天成交数量
            $startTime = mktime(0,0,0,date('m'),date('d'),date('y'));
            $endTime = mktime(23,59,59,date('m'),date('d'),date('y'));
            $maps['createAt'] = array(array('>=',$startTime),array('<=',$endTime));
            $dayzfsum = Db::name('sales')->where('sessionfpid','in',$apid)->where('status','=',1)->where($maps)->count('id');
            //当天失败数量
            $daywzfsum = Db::name('sales')->where('sessionfpid','in',$apid)->where('status','=',0)->where($maps)->count('id');
        }
