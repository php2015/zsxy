 ...
 */
class Mingxi extends AdminBase
{
    protected $ips_model;
	...
	//明细
	public function index($date1='',$date2='',$page = 1,$keyword = '')
	{
		$map = [];
        if ($keyword) {
            $map['c.kname|b.names|b.pnames'] = ['like', "%{$keyword}%"];
        }
		if ($date2=='') {
			$d2=time();
        }else
		...
		//SELECT a.`id`,b.`names`,b.`pnames`,a.`ip`,a.`dates` FROM sun_ips a,sun_user b WHERE a.`state`='点击量'
		 $ips_list  = $this->ips_model
		->alias("a")
		->field("a.id,c.kname,b.names,b.pnames,a.ip,a.dates")
		->join("__USER__ b","a.uid=b.id","LEFT")
		->join("__KOUZI__ c","a.kid=c.id","LEFT")
		->where($map)->order(['a.dates' => 'DESC'])->paginate(10, false, ['query'=>request()->param()]);
		
        return $this->fetch('index', ['ips_list' => $ips_list,'keyword' => $keyword,'date1' => $date1,'date2' => $date2]);
