...
class Chaxun extends AdminBase
{
    ...
    protected function _initialize()
    {
        ...
        public function indexs($keyword = '', $page = 1,$date1='',$date2='')
    {
    	$map = [];
		if($keyword){
			$map['a.names|a.tel'] = ['like',"%{$keyword}%"];
		}
		$map1 = [];
        ...
        if(empty($date1) && empty($date2)){
        	...
        	if($aid == 13){
        		...
        		$map['a.dates'] = [['>=',strtotime('-15 day',time())],['<=',time()]];
        	}
        }
		...
		->alias("a")
		->field("a.`id`,a.`tel`,a.`ordernumber`,a.`uid`,b.`names`,d.`agent_name`,a.`dates`,a.`pid`,c.`product_name`,a.`price`,a.`names` as namess,a.`idcard`,a.`tel`,e.names pnames,e.id suid,f.names manames,a.sheng,a.shi")
    ...
    public function index($keyword = '', $page = 1,$date1='',$date2='')
    {
        ...
        $chaxun_list  = $this->chaxun_model
		->alias("a")
		->field("a.`id`,a.`tel`,a.`ordernumber`,a.`uid`,b.`names`,d.`agent_name`,a.`dates`,a.`pid`,c.`product_name`,a.`price`,a.`names` as namess,a.`idcard`,a.`tel`,e.names pnames,e.id suid,f.names manames,f.id fuid,a.sheng,a.shi")
		->join("__USER__ b","a.uid=b.id","LEFT")
		->join("__PRODUCT__ c","a.pid=c.id","LEFT")
		->join("__AGENT__ d","b.agent_class=d.id","LEFT")
		->join("__USER__ e","b.pid=e.id","LEFT")
