...
class Yindex extends Controller
{   
    public function _initialize(){
        if(!session('uid')){
          ...
          $id=session('uid');
          $user = Db::name('user')->where('id','=',$id)->where('agent_class','>',1)->field('*')->find();
          if($user){
          }else{
            $this->redirect('index/login/ylogin');
          }
...
//apikey需要自己申请  方法见网址：   http://c7.gg/page/apidoc.html
    ...
    public function index()
    {
      ...
      $this->assign("tuikuangsun",$tuikuangsun);
        ...
        //幻灯片
        $banner = Db::name('banner')->where('id','in',['4','5','6'])->field('*')->select();
        $bannerone = Db::name('banner')->where('id','=','4')->field('*')->find();
   ...
   public function pingfen($temp_res)
    ...
    { 
      ...
      if(!empty($kstime)){
             ...
             //订单
             $dingdan=db('chaxun')->where('ma_id','=',$id)->where('dates','between',[$kstimes, $jstimes])->select();
              $dingnum=count($dingdan);
              $this->assign('dingnum',$dingnum);
              //$db->sum('profit',  'money', array('create_time','between',[$kstimes, $jstimes])); //求和
             ...
             $user  =db('profit')
               ...
               ->field("a.*,b.names,b.mobile mobilesss,b.mid,CONCAT(LEFT(c.names,1),'**') namess ,c.tel mobile")
                ...
                //->join("sun_chaxun f","a.order_id=f.id","LEFT")
                //->join("sun_user h","f.uid=h.id","LEFT")
            ...
            $this->assign('uid',$id);
            //订单
             $dingdan=db('chaxun')->where('ma_id','=',$id)->where('dates','between',[$kaishitime, $jieshutime])->select();
              $dingnum=count($dingdan);
              $this->assign('dingnum',$dingnum);
              //$db->sum('profit',  'money', array('create_time','between',[$kstimes, $jstimes])); //求和
              $profitsun=db('profit')->where('profit_id','=',$id)->where('create_time','between',[$kaishitime, $jieshutime])->where('type','neq','提现')->where('type','neq','退款')->sum('money');
              //总收益
              if(empty($profitsun)){
                $profitsun=0;
              }
              $tuikuangsun=db('profit')->where('profit_id','=',$id)->where('create_time','between',[$kaishitime, $jieshutime])->where('type','eq','退款')->sum('money');
              if(empty($tuikuangsun)){
                $tuikuangsun=0;
              }
              $this->assign('tuikuangsun',$tuikuangsun);
              //dump($tuikuangsun);die;
              //总收益
              $this->assign('profitsun',$profitsun);
               $profityiji=db('profit')->where('profit_id','=',$id)->where('create_time','between',[$kaishitime, $jieshutime])->where('type','eq','直推奖励')->sum('money');
               if(empty($profityiji)){
                  $profityiji=0;
              }
              //一级
              $this->assign('profityiji',$profityiji);
              $profiterji=db('profit')->where('profit_id','=',$id)->where('create_time','between',[$kaishitime, $jieshutime])->where('type','eq','二级奖励')->sum('money');
              if(empty($profiterji)){
                  $profiterji=0;
              }
              //二级
              $this->assign('profiterji',$profiterji);
              //er级
             $profitcont=db('profit')->where('profit_id','=',$id)->where('create_time','between',[$kaishitime, $jieshutime])->where('type','eq','二级奖励')->select();
              $profitnum=count($profitcont);
              $this->assign('profitnum',$profitnum);
            $user  =db('profit')
                ->alias("a")
                ...
                //->join("sun_chaxun f","a.order_id=f.id","LEFT")
                //->join("sun_user h","f.uid=h.id","LEFT")
                ->where('a.create_time','between',[$kaishitime, $jieshutime])
                ->where('a.profit_id','=',$id)
                ->order(['a.id' => 'DESC'])->limit(100)->select();
            ...
            for($i=0;$i<$num;$i++){
              $user[$i]["order_id"]=str_replace('T','D',$user[$i]["order_id"]);
              //$qwe=db('chaxun')->where('ordernumber','=',$user[$i]["order_id"])->find();
               $userhh  =db('user')
                ->alias("a")
                ->field("a.*")
                ->join("sun_chaxun c","a.id=c.uid","LEFT")
                ->where('c.ordernumber','=',$user[$i]["order_id"])
                ->find();
                //dump($user[$i]["order_id"]);dump($qwe);die;
              $user[$i]['nameshh']=substr_replace($userhh['names'],'**',3,6);
              $user[$i]['mobileshh']=substr_replace($userhh['mobile'],'***',4,3);
               $user[$i]['mobile']=substr_replace($user[$i]['mobile'],'***',4,3);
              $user[$i]['mobilesss']=substr_replace($user[$i]['mobilesss'],'***',4,3);
   ...
   //资金明细
  	public function mingxiss()
    {
        header("content-type:text/html;charset=utf-8");
            $id=session('uid');
            $user = db('withdraw')->where(['user_id'=>$id,'status'=>1])->order('id desc')->select();
						$alltxNum = 0;
						foreach($user as $key=>$val){
							$alltxNum += $val['money'];
							$user[$key]['zhanghao'] = substr($val['bankcard'],0,4).'******';
						}
						$this->assign('user',$user);
   ...
   public function tuiguang()
    {
        ...
        if(isset($typeId) && !empty($typeId) && $typeId == 8){
            ...
            switch($a_g_id){
                ...
                case 6:
                    $product=db('product')->where('id','=',$imgid)->find();
                    $url = 'http://'.$yuming.'/index/chaxun/query4/price/'.$product['price'].'/pid/'.$product['id'];
                    break;
                case 5:
                    $product=db('product')->where('id','=',$imgid)->find();
                    $url = 'http://'.$yuming.'/index/chaxun/query3/price/'.$product['price'].'/pid/'.$product['id'];
                    break;
                default:
                    $url = 'http://'.$yuming.'/index/chaxun/index/p_id/'.$imgid;
            }
        }
